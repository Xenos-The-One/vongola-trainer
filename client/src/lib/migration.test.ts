...
describe('migrateStore', () => {
  ...
  it('is idempotent when re-run at the current version', () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const v6 = migrateStore(structuredClone(base), 1) as any;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const again = migrateStore(structuredClone(v6), 6) as any;
    expect(again.metrics).toEqual(v6.metrics);
    expect(again.activeSession).toEqual(v6.activeSession);
    expect(again.nextLift).toBe(v6.nextLift);
    expect(again.equipmentProfile).toEqual(v6.equipmentProfile);
  });
